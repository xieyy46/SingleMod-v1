...
def read_process(database,read_name,strand,idx,motifs,memmap_sig,memmap_seq,memmap_extra,extra_file):
...
def eventalign_process(eventalign,strands,motifs,out_dir,prefix,size):
	...
	with open(eventalign,"r") as f:	
		...
		for line in f:
			...
			if info[3] == read_name:
				...
				strand = strands.get(read_name)
				idx = read_process(database,read_name,strand,idx,motifs,memmap_sig,memmap_seq,memmap_extra,extra_file)		
...
def main():
	...
	parser.add_argument('-s','--size',type=int,default=500000,required = False, help="the first dimension of memmap files; default setting is 500000, which is enough to cover 5G DRS data. If your data is over 5G, set a larger size, for example 5-10G: 1000000, 10-15G: 1500000")
	...
	eventalign_process(args.eventalign,strands,motifs,args.out_dir,args.prefix,args.size)		
